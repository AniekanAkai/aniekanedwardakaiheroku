...
class NameForm extends React.Component{
	...
	render(){
		...
		<div><b>Search results for </b><span>{(this.state.trackName)?this.state.trackName:"___"}</span> by {(this.state.artistName)?this.state.artistName:"___"} in {this.state.platformsToSearch}</div>
					...
					<div>
						<h3>Top result</h3>
						<iframe width="80%" height="166" scrolling="no" frameBorder="no" src={"https://bandcamp.com/EmbeddedPlayer/track="+topResult.id+"/transparent=true"}>
						</iframe>
					</div>
					<hr/>
					<div>
					...
					scrolling="no" frameBorder="no" src={"https://bandcamp.com/EmbeddedPlayer/track="+results.id+"/transparent=true"}>
					</iframe>
					)}</ul>
				</div>
		);
